...
class TickArenaUseCase @Inject constructor(
    ...
    suspend operator fun invoke(dtMs: Long): Result<Unit> = writeMutex.mutex.withLock {
        ...
        if (cur.pets.isEmpty()) return@withLock Result.success(Unit)
        val next = PetArenaSimulator.advance(cur, PetArenaInput.TickAll(dtMs))
